...
func ComputeStatus(name, line1, line2 string, crewCount int) (model.ISSStatus, error) {
	tleStr := name + "\n" + line1 + "\n" + line2
	tle, err := sgp4.ParseTLE(tleStr)
	if err != nil {
		return model.ISSStatus{}, fmt.Errorf("parsing TLE: %w", err)
	}
	...
	return model.ISSStatus{
		LaunchYear: 1998,
		OrbitCount: orbitCount,
		CrewCount:  crewCount,
		OrbitalParams: model.OrbitalParams{
